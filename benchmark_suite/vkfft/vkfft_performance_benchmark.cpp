...
// GPU Initializations
    ...
    results_file << "re_low,re_high,im_low,im_high,Nx,Ny,Batches,Num_Runs,GFLOPS,AverageEnergy(mJ),AverageTime(s),AveragePower(W)\n";
	...
	PrecType* buffer_input = (PrecType*)malloc((uint64_t)4 * 2 * (uint64_t)pow(2, (27)));
	if (!buffer_input) return VKFFT_ERROR_MALLOC_FAILED;
	for (uint64_t i = 0; i < (uint64_t)pow(2,27); ++i) {
		buffer_input[2 * i]     = static_cast<PrecType>((static_cast<double>(rand()) / RAND_MAX) * (real_high - real_low) + real_low);  // Real part
		buffer_input[2 * i + 1] = static_cast<PrecType>((static_cast<double>(rand()) / RAND_MAX) * (imag_high - imag_low) + imag_low);  // Imaginary part
	}
	// 1D
	// for (int N : N_values) {
	// 2D
	for (uint64_t n = 0; n < num_benchmark_samples; n++){
		...
		for (uint64_t r = 0; r < num_runs; r++) {
			...
			uint64_t bufferSize = (uint64_t)(sizeof(PrecType) * 2 * configuration.size[0] * configuration.size[1] * configuration.size[2] * configuration.numberBatches);
#if(VKFFT_BACKEND==0)
			VkBuffer buffer = {};
			...
			if (n > 0) {
				if (r == num_runs - 1) {
					...
					results_file << real_low << "," << real_high << "," << imag_low << "," << imag_high << ","
                     << configuration.size[0] << "," << configuration.size[1] << "," << configuration.numberBatches << "," << num_iter << "," << gflops << "," << avg_energy << "," << avg_time << "," << power << "\n";
