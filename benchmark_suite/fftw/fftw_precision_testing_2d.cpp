...
int main(int argc, char *argv[]) {
    ...
    int end_power = 11;  // Ending power of 2 (2^14 = 16384)
    ...
    for (const auto& size : N_values) {
        ...
        ComplexType* in_low = (ComplexType*) fftw_malloc(sizeof(ComplexType) * Nx * Ny);
        ComplexType* out_low = (ComplexType*) fftw_malloc(sizeof(ComplexType) * Nx * Ny);
    ...
    #endif
        fftw_plan p = fftw_plan_dft_2d(Nx, Ny, in_low, out_low, FFTW_FORWARD, FFTW_ESTIMATE);   
#else 
    #ifdef MULTI_THREADS
        fftwf_plan_with_nthreads(omp_get_max_threads());
    #endif
        fftwf_plan p = fftwf_plan_dft_2d(Nx, Ny, in_low, out_low, FFTW_FORWARD, FFTW_ESTIMATE);
#endif
        // Get input signal
        fftwl_complex *in_long = (fftwl_complex*) fftwl_malloc(sizeof(fftwl_complex) * Nx * Ny);
        ...
        std::vector<Complex> data_highprecision = get_fftw_values(Nx, Ny, in_long);
        ...
        results_file << real_low << "," << real_high << "," << imag_low << "," << imag_high << "," << Nx << "," << Ny << "," << abs_error << "," << rel_error*100 << "\n";
        std::cout << "Nx = " << Nx << ", Ny = " << Ny << ", Average Error = " << abs_error << std::endl;
