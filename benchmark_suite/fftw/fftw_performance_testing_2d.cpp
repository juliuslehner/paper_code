...
int main(int argc, char *argv[]) {
    ...
    // Get Arguments
    char opt_c = 0;
    while (EOF != (opt_c = getopt(argc, argv, "e:n:r:w:o:1:2:3:4:")))
    {
        switch (opt_c)
        {
        case 'e':
    ...
    results_file << "re_low,re_high,im_low,im_high,Nx,Ny,Number_Runs,Number_Repeats,GFLOPS,Total_duration,Average_duration,Power(W),Average_Energy(mJ)\n";
    ...
    for (const auto& size : N_values) {
        ...
        for (int repeat = 0; repeat < num_repeats; ++repeat) {
            ...
            ComplexType* in = (ComplexType*) fftw_malloc(sizeof(ComplexType) * Nx * Ny);
            ComplexType* out = (ComplexType*) fftw_malloc(sizeof(ComplexType) * Nx * Ny);
    ...
    #endif
            fftwf_plan p = fftwf_plan_dft_2d(Nx, Ny, in, out, FFTW_FORWARD, FFTW_ESTIMATE);
#else
    #ifdef MULTI_THREADS
            ...
            double gflops = 1.0 * (num_runs_per_plan - warmup_runs) * flop / total_duration.count() / 1000000000.0;
            ...
            std::cout << "Average FFT execution time per run: " << avg_duration << " seconds, " << "Power: " << power << "W" << std::endl;
        ...
        results_file << real_low << "," << real_high << "," << imag_low << "," << imag_high << "," << Nx << "," 
                     << Ny << "," << num_runs_per_plan << "," << num_repeats << "," << total_gflops/num_repeats << "," << total_total_duration/num_repeats << "," << total_avg_duration/num_repeats << "," << total_power/num_repeats << "," << total_avg_energy/num_repeats << "\n";
    }
