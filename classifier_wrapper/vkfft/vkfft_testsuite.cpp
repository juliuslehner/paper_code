...
VkFFTResult launchVkFFT(VkGPU* vkGPU, uint64_t sample_id, bool file_output, FILE* output, VkFFTUserSystemParameters* userParams, double real_low = -10.0, double real_high = 10.0, double imag_low = -10.0, double imag_high = 10.0, uint64_t dimension = 1) {
    ...
    case 0:
    {
        resFFT = sample_0_benchmark_VkFFT_single(vkGPU, file_output, output, real_low, real_high, imag_low, imag_high, dimension, isCompilerInitialized);
        break;
    }
    case 1:
    {
        resFFT = sample_1_benchmark_VkFFT_double(vkGPU, file_output, output, real_low, real_high, imag_low, imag_high, dimension, isCompilerInitialized);
		std::cout << resFFT << std::endl;
        break;
    }
	case 2:
    {
        resFFT = sample_2_benchmark_VkFFT_half(vkGPU, file_output, output, real_low, real_high, imag_low, imag_high, dimension, isCompilerInitialized);
        break;
    }
    case 3:
    {
    ...
    case 11:
    {
        resFFT = sample_11_precision_VkFFT_single(vkGPU, file_output, output, real_low, real_high, imag_low, imag_high, dimension, isCompilerInitialized);
        break;
    }
    case 12:
    {
        resFFT = sample_12_precision_VkFFT_double(vkGPU, file_output, output, real_low, real_high, imag_low, imag_high, dimension, isCompilerInitialized);
		std::cout << resFFT << std::endl;
        break;
    }
	case 13:
    {
        resFFT = sample_13_precision_VkFFT_half(vkGPU, file_output, output, real_low, real_high, imag_low, imag_high, dimension, isCompilerInitialized);
        break;
    }
    case 14:
    {
...
int main(int argc, char* argv[])
{
	...
	if (findFlag(argv, argv + argc, "-1"))
	{
		//select device_id
		char* value = getFlagValue(argv, argv + argc, "-1");
		if (value != 0) {
			sscanf_res = sscanf(value, "%lf", &real_low);
			if (sscanf_res <= 0) {
				printf("sscanf failed\n");
				return 1;
			}
	...
	if (findFlag(argv, argv + argc, "-2"))
	{
		//select device_id
		char* value = getFlagValue(argv, argv + argc, "-2");
		if (value != 0) {
			sscanf_res = sscanf(value, "%lf", &real_high);
			if (sscanf_res <= 0) {
				printf("sscanf failed\n");
				return 1;
			}
	...
	if (findFlag(argv, argv + argc, "-3"))
	{
		//select device_id
		char* value = getFlagValue(argv, argv + argc, "-3");
		if (value != 0) {
			sscanf_res = sscanf(value, "%lf", &imag_low);
			if (sscanf_res <= 0) {
				printf("sscanf failed\n");
				return 1;
			}
	...
	if (findFlag(argv, argv + argc, "-4"))
	{
		//select device_id
		char* value = getFlagValue(argv, argv + argc, "-4");
		if (value != 0) {
			sscanf_res = sscanf(value, "%lf", &imag_high);
			if (sscanf_res <= 0) {
				printf("sscanf failed\n");
				return 1;
			}
	...
	if (findFlag(argv, argv + argc, "-dim"))
	{
		//select device_id
		char* value = getFlagValue(argv, argv + argc, "-dim");
		if (value != 0) {
			sscanf_res = sscanf(value, "%" PRIu64 "", &dimension);
			if (sscanf_res <= 0) {
				printf("sscanf failed\n");
				return 1;
			}
	...
	if (findFlag(argv, argv + argc, "-vkfft"))
	{
		...
		if (value != 0) {
			...
			VkFFTResult resFFT = launchVkFFT(&vkGPU, sample_id, file_output, output, 0, real_low, real_high, imag_low, imag_high, dimension);
			if (resFFT != VKFFT_SUCCESS) return resFFT;
		}
		else {
