...
int main(int argc, char *argv[]) {
    ...
    uint64_t bufferSize = (uint64_t)sizeof(PrecType) * 2 * configuration.size[0];
    configuration.bufferSize = &bufferSize;
    PrecType* buffer_cpu = (PrecType*)malloc(bufferSize);
    read_binary(input_filepath, buffer_cpu, n);
...
// Allocate Memory on GPU
    ...
    #elif(VKFFT_BACKEND==4) //Level Zero APIF
    ...
    write_binary(output_filepath, buffer_cpu, n);
